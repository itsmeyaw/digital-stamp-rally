...
export default function StaffRow({
  ...
  return (
    <li
      className={`flex flex-col gap-3 border-brutal bg-white p-4 shadow-hard-sm ${member.active ? "" : "opacity-60"
        }`}
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span className={`border-[3px] border-black ${member.active ? "bg-[var(--color-accent)]" : "bg-gray-20/60"} px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-black`}>
          {member.role}
        </span>
      </div>
